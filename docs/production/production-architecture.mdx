...
3. **Key-Value Store (Consul/etcd/ZooKeeper/Kubernetes ConfigMaps):** This is the centralized store where all the configurations are stored. The key-value store ensures high availability and distributed access to the configurations. On Kubernetes, protoconf can use [ConfigMaps](./kubernetes.mdx) directly instead of running a separate store.
...
5. **Applications:** These are your actual applications that need the configurations. They connect to the protoconf agent and receive real-time configuration updates via gRPC, or read a single value with `GetConfig` over gRPC or plain HTTP.

Between step 2 and step 4, a configuration can be released gradually rather
than all at once: see [Staged Rollouts](./rollouts.mdx).
